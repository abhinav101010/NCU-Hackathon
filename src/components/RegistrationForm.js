...
export default function RegistrationForm() {
  ...
  const [formData, setFormData] = useState({
    teamName: "", teamLead: "", teamLeadEmail: "",
    teamLeadTshirt: "", phone: "", university: "", yearCourse: "",
    member1: "", member1Email: "", member1Phone: "", member1Tshirt: "",
    member2: "", member2Email: "", member2Phone: "", member2Tshirt: "",
    email: "", password: "", selectedTheme: "", ideaDescription: "",
  });
  ...
  const validatePassword = (v) => /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{6,}$/.test(v);
  ...
  const handleNextStep1 = () => {
    const e = {};
    if (!validateName(formData.teamName))       e.teamName       = "Enter a valid team name";
    if (!validateName(formData.teamLead))        e.teamLead       = "Enter a valid name";
    if (!validateEmail(formData.teamLeadEmail))  e.teamLeadEmail  = "Invalid email";
    ...
    if (formData.university.length < 3)          e.university     = "University full name required";
  ...
  const handleNextStep2 = () => {
    ...
    if (!validateEmail(formData.member1Email))   e.member1Email   = "Enter valid email";
    if (!validatePhone(formData.member1Phone))   e.member1Phone   = "Enter valid 10 digit phone";
    if (!formData.member1Tshirt)                 e.member1Tshirt  = "Select a size";
    if (!validateName(formData.member2))         e.member2        = "Enter valid name";
    if (!validateEmail(formData.member2Email))   e.member2Email   = "Enter valid email";
    if (!validatePhone(formData.member2Phone))   e.member2Phone   = "Enter valid 10 digit phone";
    if (!formData.member2Tshirt)                 e.member2Tshirt  = "Select a size";
    if (formData.member1 === formData.member2)   e.member2        = "Members must be different";
    if (formData.member1Email === formData.member2Email) e.member2Email = "Emails must be different";
    if (!validatePassword(formData.password))    e.password       = "Password must contain letters and numbers (min 6 chars)";
    setErrors(e);
  ...
  const handleSubmit = async () => {
    if (!formData.selectedTheme)   { toast.error("Please select a theme"); return; }
    if (!formData.ideaDescription) { toast.error("Describe your idea");    return; }
    setSubmitting(true);
    try {
      ...
      if (res.ok) {
        ...
        setFormData({
          teamName: "", teamLead: "", teamLeadEmail: "", phone: "",
          university: "", yearCourse: "", member1: "", member2: "",
          email: "", password: "", selectedTheme: "", ideaDescription: "",
        });
        setStep(1);
        navigate("/login");
  ...
  const backBtn = { borderRadius: "12px", py: 1.4, flexShrink: 0, minWidth: 100 };
    ...
    <Box sx={{ maxWidth: 560, mx: "auto", py: 6 }}>
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <Box sx={{ textAlign: "center", mb: 5 }}>
          <Chip label="INNOVATHON 2026 · NCU Gurugram" sx={{
            mb: 2, px: 1.5, fontSize: "0.75rem", fontWeight: 600,
      ...
      <Box sx={{ display: "flex", alignItems: "flex-start", mb: 5 }}>
        {STEPS.map((s, i) => {
          ...
          return (
            <Box key={s.num} sx={{ display: "flex", alignItems: "flex-start", flex: i < 3 ? 1 : "none" }}>
              <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 0.6 }}>
                <Box sx={{
                  ...
                  border: active ? `2px solid ${primary}`
                    : done ? "none"
                    : `1px solid ${isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)"}`,
                  color: done ? (isDark ? "#000" : "#fff")
                    : active ? primary : theme.palette.text.disabled,
                ...
                <Typography sx={{
                  fontSize: "0.6rem", fontWeight: active || done ? 700 : 400,
              ...
              {i < 3 && (
                <Box sx={{
                  flex: 1, height: "2px", mt: "17px", mx: 0.8,
                  background: done
      ...
      <Box sx={{
        ...
        <Box sx={{
          ...
          <Typography sx={{
            ...
            fontSize: "0.92rem", color: primary,
          }}>
            {STEPS[step - 1].label}
          </Typography>
          <Typography sx={{ fontSize: "0.7rem", color: "text.disabled", fontWeight: 600, letterSpacing: "0.06em" }}>
            STEP {step} / 4
          </Typography>
        </Box>
              ...
              <Box sx={{ px: 3.5, py: 3 }}>
                ...
                {step === 1 && (
                  <Box sx={{ display: "flex", flexDirection: "column", gap: GAP }}>
                    ...
                    <Field
                      ...
                      error={!!errors.teamLead} helperText={errors.teamLead}
                      onChange={(e) => updateField("teamLead", e.target.value.replace(/[^a-zA-Z\s]/g, ""))}
                    />

                    <Field
                      label="Team Lead Email"
                      value={formData.teamLeadEmail}
                      error={!!errors.teamLeadEmail} helperText={errors.teamLeadEmail}
                    ...
                    <Field
                      ...
                      error={!!errors.yearCourse} helperText={errors.yearCourse}
                      onChange={(e) => updateField("yearCourse", e.target.value)}
                    />

                    <Button fullWidth variant="contained" color="primary" onClick={handleNextStep1}
                      sx={{ ...nextBtn, mt: 1 }}>
                      Continue →
                    </Button>
                  </Box>
                ...
                {step === 2 && (
                  <Box sx={{ display: "flex", flexDirection: "column", gap: GAP }}>
                    ...
                    {[1, 2].map((m) => (
                      <Box key={m} sx={{ display: "flex", flexDirection: "column", gap: GAP }}>
                        <SectionLabel>Team Mate {m}</SectionLabel>

                        <Field
                          label={`Member ${m} Name`}
                          value={formData[`member${m}`]}
                          error={!!errors[`member${m}`]} helperText={errors[`member${m}`]}
                          onChange={(e) => updateField(`member${m}`, e.target.value.replace(/[^a-zA-Z\s]/g, ""))}
                        />

                        <Field
                          label={`Member ${m} Email`}
                          value={formData[`member${m}Email`]}
                          error={!!errors[`member${m}Email`]} helperText={errors[`member${m}Email`]}
                          onChange={(e) => updateField(`member${m}Email`, e.target.value)}
                        />

                        <Field
                          label={`Member ${m} Phone`}
                          value={formData[`member${m}Phone`]}
                          error={!!errors[`member${m}Phone`]} helperText={errors[`member${m}Phone`]}
                          onChange={(e) => updateField(`member${m}Phone`, e.target.value.replace(/\D/g, ""))}
                        />
                    ...
                    <Box sx={btnRow}>
                      <Button variant="outlined" color="primary" onClick={() => go(1)} sx={backBtn}>← Back</Button>
                      <Button variant="contained" color="primary" onClick={handleNextStep2} sx={nextBtn}>Continue →</Button>
                    </Box>
                  </Box>
                )}
                ...
                {step === 3 && (
                  <Box sx={{ display: "flex", flexDirection: "column", gap: GAP }}>
                      ...
                      <Box sx={{
                        ...
                        {themes.map((t) => {
                          ...
                          return (
                            <Box
                              key={t._id}
                              onClick={() => { setSelectedThemeObj(t); setDialogOpen(true); }}
                              sx={{
                                ...
                                boxShadow: selected ? `0 0 20px ${primary}44` : "none",
                            ...
                            >
                              <Box component="img" src={`${API}${t.img}`} alt={t.title}
                              ...
                              <Box sx={{ position: "absolute", bottom: 0, p: 1.2 }}>
                                <Typography sx={{
                                  fontFamily: "'Syne', sans-serif", fontWeight: 700,
                                  fontSize: "0.76rem", color: "#fff", lineHeight: 1.3,
                                }}>
                                  {t.title}
                                </Typography>
                              </Box>
                              {selected && (
                                <Box sx={{
                                  ...
                                  <CheckCircleIcon sx={{ fontSize: "0.8rem", color: isDark ? "#000" : "#fff" }} />
                                </Box>
                              )}
                            </Box>
                          );
                    ...
                    <Field
                      ...
                      helperText={errors.ideaDescription || `${formData.ideaDescription.length}/1000`}
                      onChange={(e) => updateField("ideaDescription", e.target.value)}
                    />

                    <Box sx={btnRow}>
                      <Button variant="outlined" color="primary" onClick={() => go(2)} sx={backBtn}>← Back</Button>
                      <Button variant="contained" color="primary" sx={nextBtn}
                        onClick={() => {
                          if (!formData.selectedTheme)   { toast.error("Please select a theme"); return; }
                          if (!formData.ideaDescription) { toast.error("Describe your idea");    return; }
                          go(4);
                ...
                {step === 4 && (
                  <Box sx={{ display: "flex", flexDirection: "column", gap: GAP }}>
                    ...
                    <Box sx={{
                      ...
                      {[
                        { label: "Registration Fee", sub: "Per team",             amount: "₹300" },
                        { label: "T-Shirts",         sub: "3 members × any size", amount: "₹500" },
                      ].map((row, i) => (
                        <Box key={i} sx={{
                          ...
                          <Box>
                            <Typography sx={{ fontSize: "0.88rem", fontWeight: 500 }}>{row.label}</Typography>
                            <Typography sx={{ fontSize: "0.7rem", color: "text.disabled" }}>{row.sub}</Typography>
                          </Box>
                          <Typography sx={{ fontWeight: 600, fontSize: "0.9rem" }}>{row.amount}</Typography>
                        </Box>
                      ))}
                      <Box sx={{
                        display: "flex", justifyContent: "space-between", alignItems: "center",
                        ...
                        <Typography sx={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "0.95rem" }}>
                          Total
                        </Typography>
                        <Typography sx={{
                          fontFamily: "'Syne', sans-serif", fontWeight: 800,
                          fontSize: "1.35rem", color: primary,
                        }}>
                          ₹800
                        </Typography>
                      </Box>
                    ...
                    <Box sx={{
                      ...
                      <Box sx={{
                        ...
                        <Box component="img" src="/payment-qr.png"
                          sx={{ width: "100%", height: "100%", objectFit: "contain" }} />
                      </Box>

                      <Box sx={{ flex: 1, display: "flex", flexDirection: "column", gap: 1.5 }}>
                        <Box>
                          <Typography sx={{ fontSize: "0.6rem", fontWeight: 700, color: "text.disabled", letterSpacing: "0.12em", mb: 0.4 }}>
                            UPI ID
                          </Typography>
                        ...
                        <Box>
                          ...
                          <Typography sx={{
                            fontFamily: "'Syne', sans-serif", fontWeight: 700,
                            fontSize: "0.88rem", color: primary,
                          }}>
                            +91 9870340530
                          </Typography>
                        </Box>
                    ...
                    <Box sx={{
                      ...
                      background: isDark ? "rgba(255,80,80,0.07)" : "rgba(255,80,80,0.06)",
                      border: "1px solid rgba(255,80,80,0.2)",
                    }}>
                      <Typography sx={{ fontSize: "0.79rem", color: "error.main", lineHeight: 1.65 }}>
                        ⚠️ Complete the payment first, then share your screenshot with the contact above. Registration is confirmed only after verification.
                      </Typography>
                    </Box>

                    <Box sx={btnRow}>
                      <Button variant="outlined" color="primary" onClick={() => go(3)} sx={backBtn}>← Back</Button>
                      <Button variant="contained" color="secondary"
                        disabled={submitting || calculateTimeLeft().expired}
                        onClick={handleSubmit} sx={nextBtn}>
                        {calculateTimeLeft().expired ? "Registrations Closed"
        ...
        {selectedThemeObj && (
          <>
            <Box sx={{ position: "relative", height: 190 }}>
              <Box component="img" src={`${API}${selectedThemeObj.img}`} alt={selectedThemeObj.title}
            ...
            <DialogActions sx={{ px: 3, pb: 3, gap: 1 }}>
              <Button onClick={() => setDialogOpen(false)} variant="outlined"
                sx={{ borderRadius: "11px" }}>Close</Button>
              <Button variant="contained" color="primary" sx={{ borderRadius: "11px" }}
                onClick={() => { updateField("selectedTheme", selectedThemeObj.title); setDialogOpen(false); }}>
                Select Theme
              </Button>
            </DialogActions>
          </>
