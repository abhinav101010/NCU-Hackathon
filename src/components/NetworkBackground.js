...
function detectTheme(theme) {
  ...
  if (p === "#1c6bba" || p === "#0066cc" || p === "#0053a0") return "wind";
  return "light";
}
...
function runWind(canvas, ctx) {
  ...
  const draw = () => {
    ...
    // road reflection glow (blue from bottom)
    const roadGlow = ctx.createLinearGradient(0, H*0.65, 0, H);
    roadGlow.addColorStop(0, "rgba(10,40,90,0)");
    roadGlow.addColorStop(0.6, "rgba(14,50,110,0.08)");
    roadGlow.addColorStop(1, "rgba(20,70,160,0.18)");
    ctx.fillStyle = roadGlow; ctx.fillRect(0, 0, W, H);

    // top blue highlight
    const topHL = ctx.createLinearGradient(0, 0, 0, H*0.3);
    topHL.addColorStop(0, "rgba(28,107,186,0.06)");
    topHL.addColorStop(1, "rgba(28,107,186,0)");
